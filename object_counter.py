    ...
    def extract_and_process_tracks(self, tracks):
        ...
        if tracks[0].boxes.id is not None:
            ...
            for box, track_id, cls, conf in zip(boxes, track_ids, clss, confs):
                ...
                if len(self.p_xyxy) > 0:
                    p_x1, p_y1, p_x2, p_y2 = self.p_xyxy[-1].tolist()
                    p_top_left = (p_x1, p_y1)
                    p_top_right = (p_x2, p_y1)
                    p_bottom_left = (p_x1, p_y2)
                    p_bottom_right = (p_x2, p_y2)
                    
                if(self.id_location_mapper.get(track_id) is None):
                ...
                # ---------------------------- EDIT THIS TO COUNT SAME PERSON GOING ONE DIRECTION, AND THEN BACK THE OTHER DIRECTION -------------------------
                elif len(self.reg_pts) == 2:
                    if prev_position is not None and len(self.p_xyxy) > 0:
                        distance = Point(track_line[-1]).distance(self.counting_region)
                        if (p_top_left[0] < top_left[0] and p_top_left[0] > self.reg_pts[0][0] and self.id_location_mapper.get(track_id) is not None and self.id_location_mapper[track_id] != "right"):
                            self.in_counts += 1
                            self.class_wise_count[self.names[cls]]["IN"] += 1
                            self.id_location_mapper[track_id] = "right"
                        elif (p_top_right[0] > top_right[0] and p_top_right[0] < self.reg_pts[0][0] and self.id_location_mapper.get(track_id) is not None and self.id_location_mapper[track_id] != "left"):
                            self.out_counts += 1
                            self.class_wise_count[self.names[cls]]["OUT"] += 1
                            self.id_location_mapper[track_id] = "left"
                self.p_xyxy.append(box)
                if(len(self.p_xyxy)>30):
                    self.p_xyxy.pop(0)           
