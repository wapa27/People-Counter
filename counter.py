...
#     parser.add_argument('--output_path', type=str, default=None, help="Provide the desired output path for saved frames")
#     parser.add_argument('--save_interval', type=int, default=1, help="Provide the desired interval (in seconds) to save frames")
#     parser.add_argument('--write_batch_interval', type=int, default=5, help="Provide the desired interval (in seconds) for writing the batch to memory")
#     opt = parser.parse_args()
#     if opt.save_interval > opt.write_batch_interval:
#         print("Save Interval must be less than Write Batch Interval")
#         exit(1)
...
#         print(str(time.time()) + "  ::   " + str(last_person_seen))
#         if (not isAnyoneInFrame) and (time.time() - last_person_seen) > 5:
#             # If no one has been in the frame for more than 5 seconds
#             print("No one in frame for 5 seconds")
#             is_time_reached = True


# async def main():
#     global isAnyoneInFrame, last_person_seen, is_time_reached, has_timer_started, has_timer_ended
#     logging.getLogger('ultralytics').setLevel(logging.WARNING)
#     # _____________ZED CONFIGURATION_______________
#     line_points = []
#     line_direction = opt.line_direction
...
#                 # print('no one in frame, starting timer')
#                 timer = asyncio.create_task(monitor_ifAnyoneInFrame())
